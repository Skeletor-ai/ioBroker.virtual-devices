...
class VirtualDevicesManagement extends DeviceManagement {
    ...
    async addDevice(context) {
        ...
        if (selectedRoom) {
            await this._assignRoom(`${this.adapter.namespace}.devices.${deviceId}`, selectedRoom);
        }
    ...
    async editDevice(deviceId, context) {
        ...
        const fullObjId = `${this.adapter.namespace}.devices.${deviceId}`;
    ...
    async deleteDevice(deviceId, context) {
        ...
        const objId = `${this.adapter.namespace}.devices.${deviceId}`;
        const obj = await this.adapter.getObjectAsync(`devices.${deviceId}`);
        if (obj?.native?.room) {
