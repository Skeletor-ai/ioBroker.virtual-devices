...
function buildObjectIdPicker(slot) {
    ...
    // Build filterFunc — a JS expression string evaluated per object.
    // Return true = show object, false = hide.
    // Using filterFunc instead of customFilter for unambiguous inclusion logic.
    const conditions = [];

    if (slot.filter.common) {
        if (slot.filter.common.type) {
            conditions.push(`obj.common && obj.common.type === '${slot.filter.common.type}'`);
        }
        if (slot.filter.common.role) {
            const roles = Array.isArray(slot.filter.common.role)
                ? slot.filter.common.role
                : [slot.filter.common.role];
            // Prefix matching (like customFilter): role 'switch' matches 'switch', 'switch.power', etc.
            const roleChecks = roles.map(
                (r) => `(obj.common.role && obj.common.role.startsWith('${r}'))`
            );
            conditions.push(`(${roleChecks.join(' || ')})`);
        }
        if (slot.filter.common.unit) {
            conditions.push(`obj.common && obj.common.unit === '${slot.filter.common.unit}'`);
        }
    }

    if (conditions.length > 0) {
        item.filterFunc = conditions.join(' && ');
    }
