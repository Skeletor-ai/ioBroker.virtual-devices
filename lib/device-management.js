...
class VirtualDevicesManagement extends DeviceManagement {
    ...
    async listDevices() {
        ...
        for (const row of objs.rows) {
            ...
            // dm-utils requires handler (set) and getStateHandler (get) on each control.
            /** @type {object[]} */
            const controls = [];
            if (plugin) {
                for (const out of plugin.outputStates) {
                    if (out.write && out.type === 'boolean') {
                        ...
                        controls.push({
                            ...
                            channel: {
                                name: out.description || out.name,
                            },
                            getStateHandler: async (_devId, _controlId) => {
                                const state = await adapter.getStateAsync(stateId);
                                return state || { val: false, ack: true, ts: Date.now() };
                            },
                            handler: async (_devId, _controlId, newState) => {
                                await adapter.setStateAsync(stateId, { val: newState, ack: false });
                                return { val: newState, ack: true, ts: Date.now() };
                            },
                        });
                    }
                }
