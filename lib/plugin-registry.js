...
async function loadBuiltInPlugins() {
    ...
    registerPlugin(new ConditionalSwitchPlugin());

    const { BathroomFanPlugin } = require('../plugins/bathroom-fan');
    registerPlugin(new BathroomFanPlugin());

    const { HomeCinemaPlugin } = require('../plugins/home-cinema');
    registerPlugin(new HomeCinemaPlugin());
}
